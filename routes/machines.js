...
*/
exports.machinesGetId = function (req, res) {
    var app = require('../common');
    var id = new ObjectID(req.params.id);
    GetMachinesId(app.getDB(), { _id: id }, function (data) {
        res.contentType('json');
        res.json({
            success: true,
            machines: data
...
function UpdateMachines(db, _id, data, callback) {
    db.collection('machines', function (err, collection) {
        collection.update({ _id: _id }, { $set: data }, { safe: true }, function (err) {
...
* AUTHOR : VEDAVITH RAVULA
*/
function GetMachinesId(db, query, callback) {
    var machines = [];
